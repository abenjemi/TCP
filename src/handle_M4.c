...
uint32_t ring_buffer_read(ring_buffer_t* buffer, char* dst_buff, uint32_t bytes, uint32_t index)
{
    if (buffer == NULL) {
    ...
    uint32_t start_idx = (buffer->head+index) % buffer->capacity;
    ...
    while (1) {
        ...
        for (int i = 0; i < tinytcp_conn_list_size; ++i) {
            ...
            || tinytcp_conn->curr_state == READY_TO_TERMINATE) {
                ...
                uint32_t data_size = ring_buffer_read(tinytcp_conn->send_buffer, data, MSS, 0);
                ...
                if (data_size > 0)
                {
                    ...
                    {
                        int j = 0;
                        last_ack[i] = tinytcp_conn->seq_num + MSS;
                        while (j < 10)
                        {
                            ...
                            uint32_t idx = j*MSS + data_size;
                            if (occupied_space(tinytcp_conn->send_buffer, &idx) == 0)
                            {
                                break;
                            }
                            while(occupied_space(tinytcp_conn->send_buffer, NULL) < (MSS*(j+2)) && tinytcp_conn->curr_state != READY_TO_TERMINATE);
                            j++;
                            data_size = ring_buffer_read(tinytcp_conn->send_buffer, data, MSS, MSS * (j+1)); 
                        }
                    ...
                    if (timer_expired(tinytcp_conn->time_last_new_data_acked) || tinytcp_conn->num_of_dup_acks == 3) {
                        int k = 0;
                        seq_number = last_ack[i] + MSS;
                        while (seq_number <= tinytcp_conn->seq_num)
                        {
                            data_size = ring_buffer_read(tinytcp_conn->send_buffer, data, MSS, MSS * k); 
                            char * data_pkt = create_tinytcp_pkt(tinytcp_conn->src_port,
                            tinytcp_conn->dst_port, tinytcp_conn->seq_num,
                            tinytcp_conn->ack_num, 1, 0, 0, data, data_size);
                            send_to_network(data_pkt, TINYTCP_HDR_SIZE + data_size);
        ...
        if (tinytcp_conn->curr_state == SYN_ACK_SENT) { //conn set up ACK
            ...
            else
            {
                ...
                if (ack_num == last_ack[index] + MSS)
                {
                    ...
                    uint32_t bytes_read = ring_buffer_remove(tinytcp_conn->send_buffer, dst_buffer, MSS);
                    free(dst_buffer);
                    last_ack[index] = ack_num;
                    if (tinytcp_conn->seq_num == ack_num - MSS)
                    {
                        send_now[index] = 1;
                    }
                }
                else if (ack_num == last_ack[index])
                {
